...
export default function ProductCard({
  ...
  const numOriginal = originalPrice ? (typeof originalPrice === 'string' ? parseFloat(originalPrice) : originalPrice) : null;
  const discount = numOriginal ? Math.round((1 - numPrice / numOriginal) * 100) : null;
  const imageUrl = images?.[0] || `https://placehold.co/400x400/1A1A1A/C8102E?text=${encodeURIComponent(name.split(' ')[0])}`;

  const genderLabel: Record<string, string> = { masculino: 'Masculino', feminino: 'Feminino', infantil: 'Infantil' };
  const categoryLabel: Record<string, string> = { times: 'Times', selecoes: 'Seleções', retro: 'Retrô' };
  ...
  const handleQuickAdd = (e: React.MouseEvent) => {
    ...
    toast.success(`${name} adicionado ao carrinho!`, {
      description: 'Tamanho M selecionado. Altere na página do produto.',
      duration: 3000,
    });
  };
  ...
  return (
    <Link href={`/produto/${slug}`}>
      <div className="product-card bg-[#111111] rounded-xl overflow-hidden cursor-pointer group border border-[#1E1E1E] hover:border-[#C8102E]/40">
        ...
        <div className="relative aspect-square overflow-hidden bg-[#1A1A1A]">
          ...
          <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-center pb-4">
            <button
              ...
              className="flex items-center gap-2 bg-[#C8102E] hover:bg-red-700 text-white text-sm font-bold px-4 py-2 rounded-lg transition-all transform translate-y-2 group-hover:translate-y-0"
            >
        ...
        <div className="p-3">
          {team && (
            <p className="text-[#C8102E] text-xs font-semibold uppercase tracking-wider mb-1">{team}</p>
          )}
          ...
          <div className="flex items-center gap-1 mb-2">
            <span className="text-gray-600 text-[10px]">{genderLabel[gender]} • {categoryLabel[category]}</span>
          </div>
