...
import { Award, TrendingUp, DollarSign, ShoppingBag, Calendar, Percent, Download } from "lucide-react";
...
export default function PdvComissoes() {
  ...
  function exportCSV() {
    ...
    const header = "Vendedor,Pedidos,Faturamento,Atacado,Varejo,Balcão,WhatsApp,Ticket Médio,Comissão,Meta\n";
    const rows = sellers.map(s =>
      `${s.sellerName},${s.totalPedidos},${s.faturamento.toFixed(2)},${s.faturamentoAtacado.toFixed(2)},${s.faturamentoVarejo.toFixed(2)},${s.faturamentoBalcao.toFixed(2)},${s.faturamentoWhatsapp.toFixed(2)},${s.ticketMedio.toFixed(2)},${s.comissao.toFixed(2)},${s.metaAtingida || "Sem meta"}`
    ).join("\n");
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-0.5">Desempenho e comissões por vendedor</p>
          </div>
          <button
            onClick={exportCSV}
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4 flex flex-wrap items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <label className="text-gray-400 text-sm">Taxa de comissão:</label>
            <input
              type="number"
              min={0}
              max={100}
          ...
          <>
            ...
            {sellers.length > 0 && (
              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
                <h3 className="text-white font-semibold mb-4">Faturamento por Vendedor</h3>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-gray-800">
                        ...
                        <th className="text-right text-gray-400 text-xs font-semibold px-4 py-3">Ticket Médio</th>
                        <th className="text-right text-gray-400 text-xs font-semibold px-4 py-3">Comissão ({taxaComissao}%)</th>
                      ...
                      {sellers.map((s, i) => (
                        <tr key={s.sellerId} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                          ...
                          <td className="px-4 py-3 text-right text-blue-400 text-sm">
                            {formatCurrency(s.faturamentoAtacado)}
                          </td>
                          <td className="px-4 py-3 text-right text-orange-400 text-sm">
                            {formatCurrency(s.faturamentoVarejo)}
                          </td>
                          <td className="px-4 py-3 text-right text-gray-400 text-sm">
                            {formatCurrency(s.ticketMedio)}
                          </td>
                          <td className="px-4 py-3 text-right text-yellow-400 font-semibold text-sm">
                    ...
                    <tfoot>
                      <tr className="border-t border-gray-700 bg-gray-800/30">
                        ...
                        <td className="px-4 py-3 text-right text-white font-bold text-sm">
                          {formatCurrency(summary?.totalFaturamento || 0)}
                        </td>
                        <td className="px-4 py-3 text-right text-blue-400 font-semibold text-sm">
                          {formatCurrency(sellers.reduce((a, s) => a + s.faturamentoAtacado, 0))}
                        </td>
                        <td className="px-4 py-3 text-right text-orange-400 font-semibold text-sm">
                          {formatCurrency(sellers.reduce((a, s) => a + s.faturamentoVarejo, 0))}
                        </td>
                        <td className="px-4 py-3" />
                        <td className="px-4 py-3 text-right text-yellow-400 font-bold text-sm">
                          {formatCurrency(summary?.totalComissoes || 0)}
                        </td>
                        <td className="px-4 py-3" />
                      </tr>
                    </tfoot>
