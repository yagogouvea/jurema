              ...
              if (totalDinheiro > 0) {
                ...
                if (dbCash) {
                  try {
                    ...
                    }).catch((err: any) => console.error('[PDV Orders] Erro na notificação de suprimento:', err));
  ...
  list: publicProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      try {
        ...
        if (input.startDate) { whereClause += ` AND DATE(${spLocalDateTimeExpr("o.createdAt")}) >= ?`; params.push(input.startDate); }
        if (input.endDate) { whereClause += ` AND DATE(${spLocalDateTimeExpr("o.createdAt")}) <= ?`; params.push(input.endDate); }
        if (input.search) {
  ...
  caixinhasReport: publicProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      try {
        ...
        if (input.startDate) { histQuery += ` AND DATE(${spLocalDateTimeExpr("s.createdAt")}) >= ?`; histParams.push(input.startDate); }
        if (input.endDate) { histQuery += ` AND DATE(${spLocalDateTimeExpr("s.createdAt")}) <= ?`; histParams.push(input.endDate); }
        ...
        if (seller.role === 'admin') {
          ...
          if (input.endDate) { sumQuery += ` AND DATE(${spLocalDateTimeExpr("s.createdAt")}) <= ?`; sumParams.push(input.endDate); }
        ...
        if (input.startDate) { totalQuery += ` AND DATE(${spLocalDateTimeExpr("s.createdAt")}) >= ?`; totalParams.push(input.startDate); }
        if (input.endDate) { totalQuery += ` AND DATE(${spLocalDateTimeExpr("s.createdAt")}) <= ?`; totalParams.push(input.endDate); }
