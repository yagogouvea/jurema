...
import { resolverConflitosDescricao } from "./pdvProductCode";
...
// A coluna estoque é INT (máx ~2,1 bi). Produtos "infinitos" vêm com números
// astronômicos na planilha (ex.: 1e11) que estouravam o INT e faziam o INSERT
// falhar. Limita a um teto seguro = "ilimitado" na prática.
...
// A lógica vive em ./pdvProductCode (módulo único compartilhado com pdvAutoSync).
  ...
  for (const p of validRaw) {
    ...
    if (existing) {
      ...
      deduped.set(p.codigo, {
        ...p,
        estoque: clampEstoque(existing.estoque + p.estoque),
